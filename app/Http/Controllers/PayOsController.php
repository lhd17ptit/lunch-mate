...
class PayOsController extends Controller
{
	...
	{
		$payOsUrl = $this->payOsService->processPayment($request);
		if ($request->ajax()) {
			return response()->json([
				'redirect_url' => $payOsUrl
			]);
		}
		return redirect()->away($payOsUrl);
	}
    ...
    public function testProcess(Request $request){
        ...
        $inputData = [
            ...
            "returnUrl" => URL::route('vnpay.return', [], true),
            "cancelUrl" => URL::route('vnpay.return', [], true),
	...
	{
		$data = $this->payOsService->getReturnData($request);
